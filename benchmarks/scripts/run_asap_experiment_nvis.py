...
ASAP_PATH = ROOT_DIR / "asap" / "python"
...
def run_single_nvis_trial(subject_id: str, trial_num: int, scene: str, ref_path: str,
                          left_cond: dict, right_cond: dict):
    """
    Executes a single 2AFC presentation trial using the Nvidia player (nvis).
    """
    ...
    # Launch player in the foreground, blocking until closed
    subprocess.run(cmd, cwd=".", env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    resp_time = time.time() - start_time
    
    # Now prompt the user in the console
    print("\n" + "="*60)
    print(f"  TRIAL #{trial_num} | SCENE: {scene.upper()}")
    print("  Which video stream looked better (less artifacts)?")
    print("    [A] Choose Left  (Condition: " + left_cond["name"] + ")")
    print("    [D] Choose Right (Condition: " + right_cond["name"] + ")")
    print("    [Q] Abort/Quit Experiment")
    print("="*60)
    
    choice = None
    while True:
        try:
            inp = input("  Your choice (A/D/Q): ").strip().upper()
            if inp in ('A', 'LEFT'):
                choice = 'LEFT'
                break
            elif inp in ('D', 'RIGHT'):
                ...
                print("  Invalid input! Press 'A' for Left, 'D' for Right, or 'Q' to Quit.")
        except (KeyboardInterrupt, EOFError):
            choice = None
            break
            
    # Estimate actual presentation frame rate
    fps = 240.0
    return choice, resp_time, fps
...
def main():
    ...
    print(f"Visualizer Engine : {args.player.upper()}", flush=True)
    ...
    while completed_trials < target_total:
        ...
        if args.player == "nvis":
            print("  [Nvis Visualizer Active - Compare streams, then close window (Esc) when ready]...", flush=True)
            choice, resp_time, fps = run_single_nvis_trial(
