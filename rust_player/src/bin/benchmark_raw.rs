...
fn get_raw_dataset_dir() -> PathBuf {
    if let Ok(env_val) = std::env::var("GAIM240_RAW_DIR") {
...
fn main() {
    ...
    let dataset_dir = get_raw_dataset_dir();
    if !dataset_dir.exists() {
        println!("Error: Raw RGB dataset directory not found at {:?}", dataset_dir);
        return;
    }
    ...
    println!("Left Stream Path: {:?}", left_path);
    println!("Ref Stream Path : {:?}", ref_path);
    println!("Right Stream Path: {:?}", right_path);
    println!(
        "VSync / Pacer   : {}",
        if use_pacer {
    ...
    // 1. Load the raw concatenated RGB files directly from disk into RAM
    println!("Step 1: Reading .rgb files from disk into system RAM...");
    let t_load_start = Instant::now();

    // Use parallel spawns to read the 3 files concurrently
    let (left_data, (ref_data, right_data)) = rayon::join(
        || fs::read(&left_path).expect("Failed to read left stream"),
        || rayon::join(
            || fs::read(&ref_path).expect("Failed to read ref stream"),
            || fs::read(&right_path).expect("Failed to read right stream")
        )
    );

    let t_load = t_load_start.elapsed().as_secs_f64();
    let num_frames = left_data.len() / FRAME_SIZE;
    let total_ram_mb = (left_data.len() + ref_data.len() + right_data.len()) as f64 / 1024.0 / 1024.0;
    
    println!(
        "Load Complete: Loaded {} frames in {:.4}s ({:.2} MB in system RAM)",
        num_frames * 3,
        t_load,
        total_ram_mb
    );
    ...
    let (mut window, events) = glfw.with_connected_monitors(|glfw_ref, monitors| {
        if let Some(mon) = monitors.first() {
            glfw_ref
                .create_window(mon_w, mon_h, "GAIM240 Concatenated Raw RGB24", glfw::WindowMode::FullScreen(mon))
                .unwrap()
        } else {
            glfw_ref
                .create_window(1280, 720, "GAIM240 Concatenated Raw RGB24", glfw::WindowMode::Windowed)
                .unwrap()
        }
    });
    ...
    while !window.should_close() && step < num_frames && !quit {
        ...
        let frame_left = &left_data[offset .. offset + FRAME_SIZE];
        let frame_ref = &ref_data[offset .. offset + FRAME_SIZE];
        let frame_right = &right_data[offset .. offset + FRAME_SIZE];
