...
fn main() {
    ...
    for row in &batch_trials {
        ...
        if tf.left_stream.num_frames != tf.ref_stream.num_frames || tf.ref_stream.num_frames != tf.right_stream.num_frames {
            println!(
                "  [Sync] Trial #{}: Stream frame counts differ (Left: {}, Ref: {}, Right: {}). Synchronized playback to shortest ({} frames).",
                trial_num, tf.left_stream.num_frames, tf.ref_stream.num_frames, tf.right_stream.num_frames, min_frames
            );
        }
